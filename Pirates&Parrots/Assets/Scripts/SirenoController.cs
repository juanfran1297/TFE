...
public class SirenoController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isAttack == false)
        {
            ...
            sirenoAnim.SetBool("VistaPlayer", false);
        }

        else if (isAttack == true)
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(player.x - 1f, transform.position.y, player.z), attackSpeed * Time.deltaTime);
            transform.LookAt(new Vector3(player.x, transform.position.y, player.z));
            sirenoAnim.SetBool("VistaPlayer", true);
        }
